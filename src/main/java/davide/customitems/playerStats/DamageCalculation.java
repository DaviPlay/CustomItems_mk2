...
public class DamageCalculation implements Listener {
    ...
    public static Damage getTotalDamage(ItemStack is, Player player) {
        ...
        int totalDamage = Math.max((Item.getDamage(is) + armorDamage), 0);
        ...
        if (meta.hasEnchants()) {
            if (meta.getEnchants().containsKey(Enchantment.DAMAGE_ALL))
                ...
                totalDamage += (int) (totalDamage * (double) percentage / 100);
            }
        }
    ...
    private void onMeleeHit(EntityDamageByEntityEvent e) {
        ...
        Damage damage = getTotalDamage(is, player);
